Generate ONLY the Gherkin feature file for a Jira story — no step definitions, no Vansah import.
Covers all scenario categories. Automatable and Non-Automatable scenarios are tagged correctly.
...
Scenario: <title>
```
The `@ignore` tag causes SpecFlow to skip the scenario entirely.
...
Output:
```
  Feature file created : vFluxAutomation/[FEATURE_PATH]/[FEATURE_NAME].feature
  Total scenarios      : N
