Generate BDD scenarios + step definitions for ANY framework/language AND immediately import to Vansah using vansah-import.exe — all in one shot.
...
```
If created from scratch, stop and ask the user to fill in VansahToken and ProjectKey before proceeding to import.

---

## Step 9 — Import to Vansah

Run vansah-import.exe:

```bash
vansah-import [FEATURE_PATH]/[FEATURE_NAME].feature
```

If vansah-import.exe is not on PATH, use full path:
```bash
...
- Test case IDs created in Vansah
...
```
  ...
  Vansah import:             N created, 0 failed
